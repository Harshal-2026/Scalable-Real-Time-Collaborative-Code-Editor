...
function getGroqClient() {
    if (!groq) {
        ...
        if (!apiKey) throw new Error("GROQ_API_KEY environment variable is not set. Please add it in your Render dashboard.");
...
app.post("/api/ai", async (req, res) => {
    try {
        ...
        const completion = await groqClient.chat.completions.create({
            model: "llama-3.1-8b-instant", // Supported stable model
            messages: [
                {
                    role: "system",
